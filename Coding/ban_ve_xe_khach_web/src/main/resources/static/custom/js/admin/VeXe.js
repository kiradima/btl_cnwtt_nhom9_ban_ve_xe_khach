$(document).ready(function () {
    ...
    let getData = function (requestData, renderFunction) {
        ...
        jQuery.get("/api/v1/VeXe/list", params, function (response) {
            let content = {
    ...
    $(document).on("click", "#id-btn-add", function (e) {
        ...
        let VeXe = {
            "id": $('#id').val(),
            "mucGia": $('#mucGia').val(),
            "viTriGhe": $('#viTriGhe').val(),
            "idChuyenXe": $('#idChuyenXe').val(),
        };

        $.ajax({
            url: "/api/v1/VeXe/create",
            type: 'POST',
            contentType: "application/json; charset=utf-8",
            data: JSON.stringify(VeXe),
            error: function () {
    ...
    $(document).on("click", ".btn-delete", function () {
        result_confirm = confirm("Are you sure want to DELETE this VeXe");
        if (result_confirm == true) {
            $.ajax({
                url: "/api/v1/VeXe/delete/" + $(this).attr('id'),
                type: 'DELETE',
                error: function () {
                    window.component.alert.show("error", "Error", 2000);
                },
    ...
    $(document).on("click", ".btn-edit", function () {
        $.ajax({
            ...
            success: function (data) {
                console.log(data);
                $('#id_edit').val(data.id);
                $('#mucGia_edit').val(data.mucGia);
                $('#viTriGhe_edit').val(data.viTriGhe);
    ...
    $(document).on("click", "#id-btn-update", function () {
        let VeXe = {
            "id": $('#id_edit').val(),
            "mucGia": $("#mucGia_edit").val(),
            "viTriGhe": $("#viTriGhe_edit").val(),
            "idChuyenXe": $("#idChuyenXe_edit").val(),
        };

        $.ajax({
            url: "/api/v1/VeXe/update",
            type: 'PUT',
            contentType: "application/json; charset=utf-8",
            data: JSON.stringify(VeXe),
            error: function () {
